//
//  KeyValueObservation.mm
//
//
//  Created by Jinwoo Kim on 6/11/23.
//

#import "KeyValueObservation.hpp"
...
- (instancetype)initWithObject:(id)object forKeyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options callback:(void (^)(id _Nonnull, NSDictionary * _Nonnull))callback {
    if (self = [self init]) {
        ...
        objc_setAssociatedObject(object, _associationKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)invalidate {
    ...
    objc_setAssociatedObject(self.object, _associationKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    self.object = nil;
}
